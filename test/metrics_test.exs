defmodule Hui.MetricsTest do
  ...
  test "metrics/2 handles the configured default URL" do
  ...
  test "metrics/2 handles the atomic URL" do
  ...
  test "metrics/2 handles the binary URL", %{bypass: bypass} do
  ...
  test "metrics/2 makes the correct request given metrics options", %{bypass: bypass} do
    metrics_url = "http://localhost:#{bypass.port}/solr/admin/metrics"
  ...
  test "metrics/2 parses JSON response", %{bypass: bypass} do
    metrics_url = "http://localhost:#{bypass.port}/solr/admin/metrics"
  ...
  test "metrics/2 parses non-JSON response as raw text", %{bypass: bypass} do
    metrics_url = "http://localhost:#{bypass.port}/solr/admin/metrics"
