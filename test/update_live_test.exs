defmodule HuiUpdateLiveTest do
  ...
  describe "update! (live)" do
    ...
    test "should post a single doc (Map)" do
      ...
      delete_verify_doc_deletion(
        url,
        %Query.Update{delete_id: "tt0042876", commit: true},
        "tt0042876"
      )
    ...
    test "should post multiple docs (Map)" do
      ...
      delete_verify_doc_deletion(
        url,
        %Query.Update{delete_id: ["tt0046438", "tt1226236"], commit: true},
        ["tt0046438", "tt1226236"]
      )
    ...
    test "should post binary data" do
      ...
      Hui.update!(url, File.read!("./test/data/delete_doc2.xml"))
      Hui.update!(url, "<commit/>")

      resp = Hui.search!(:default, q: "*", fq: ["id:9781910701874"])
      assert resp.body["response"]["numFound"] == 0

      Hui.update!(url, File.read!("./test/data/update_doc2.xml"))
      Hui.update!(url, "<commit/>")
